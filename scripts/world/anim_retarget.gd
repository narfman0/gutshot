## Retargets classic Polygon-rig animation clips (Base Locomotion / Sword
## Combat packs) onto Synty character skeletons. Position/scale tracks are
## dropped (different unit conventions between the rigs); rotations are
## retargeted per joint.
##
...
## bone-local axes ~90° apart; skipping it applies every rotation around the
## wrong axis and corkscrews the whole skeleton.
##
...
##
## Target rigs are auto-detected: skeletons with a "Pelvis" bone use the
## Unreal-style BONE_MAP; skeletons that carry the classic rig's own "Hips"
## naming use an identity map (the math degenerates to a straight copy).
class_name AnimRetarget
extends RefCounted

## classic-rig bone → Unreal-style (SK_Character_* / SK_Chr_*) bone.
## Unmapped bones are dropped.
const BONE_MAP := {
	"Hips": "Pelvis",
...
static func map_for(target_skeleton: Skeleton3D) -> Dictionary:
	if target_skeleton.find_bone("Pelvis") >= 0:
		return BONE_MAP
	# Classic rig already — identity over the same bone names.
	var ident := {}
...
static func _load_reference() -> void:
	...
	for i in skel.get_bone_count():
		...
		_ref_rests[bone_name] = skel.get_bone_rest(i).basis.get_rotation_quaternion()
		_ref_global_rests[bone_name] = _global_rest_quat(skel, i)
	inst.free()
...
static func load_clip(glb_path: String, skeleton_path: String, target_skeleton: Skeleton3D) -> Animation:
	...
	_load_reference()
	var bone_map := map_for(target_skeleton)
