...
func setup(squad: Squad, level_root: Node3D, floor_heights: Array) -> void:
	_squad = squad
	heights = floor_heights
	for i in heights.size():
		_geometry.append([])
		_lights.append([])
		_display.append(1.0 if i == 0 else 0.0)
...
func _process(delta: float) -> void:
	...
	for i in heights.size():
		...
		if i <= active_floor:
			...
			target = smoothstep(REVEAL_START, REVEAL_END, climb_t(active.global_position.y))
...
## one floor above; deeper floors hide everyone with the geometry.
func _update_characters() -> void:
	...
	for node in get_tree().get_nodes_in_group("characters"):
		...
		if floor_idx <= active_floor or c.team == 0:
			c.visible = floor_idx <= active_floor + 1
		elif floor_idx == active_floor + 1:
			c.visible = _crew_sees(c, crew)
		else:
			c.visible = false
